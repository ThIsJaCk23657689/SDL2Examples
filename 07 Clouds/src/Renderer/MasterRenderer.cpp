...
MasterRenderer::MasterRenderer() {
    ...
    cloud_shader = std::make_unique<CloudShader>();
    ...
    cloud_renderer = std::make_unique<CloudRenderer>(cloud_shader.get());
...
void MasterRenderer::Render(const std::unique_ptr<Camera>& camera) {
    ...
    lightning_renderer->Prepare(camera.get());
...
//    normal_map_renderer->Render(state.world->earth, state.world->my_sphere.get(), &TextureManager::GetTexture2D("Earth Normal"));
    ...
    // 繪製光球 這邊設計應該可以再更好
    basic_renderer->Prepare(camera.get());
...
//    }
    ...
    if (state.world->draw_axes) {
        axes_renderer->Prepare(camera.get());
    ...
    cloud_renderer->Prepare(camera.get());
    cloud_renderer->Render(world->my_cloud.get());
}
