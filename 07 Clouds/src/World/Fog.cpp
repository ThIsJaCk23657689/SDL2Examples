...
Fog::Fog(glm::vec3 col, float des, float grad, float f_s, float f_e, bool ena) :
    mode(FogMode::EXP), color(col), density(des), gradient(grad), start(f_s), end(f_e), enable(ena) {
}
...
void Fog::Set(std::unique_ptr<Shader>& shader) {
    ...
    shader->SetFloat("fog.gradient", gradient);
