...
std::map<std::string, Texture2DMS> TextureManager::texture2DMSs;
...
//    TextureManager::CreateTexture2D("brickwall_normal.jpg", "Brick Wall Normal", false, false);
    ...
    std::vector<std::string> sky_box = {
        "assets/textures/daylight/1.bmp",
        "assets/textures/daylight/2.bmp",
        "assets/textures/daylight/3.bmp",
        "assets/textures/daylight/4.bmp",
        "assets/textures/daylight/5.bmp",
        "assets/textures/daylight/6.bmp"
    };
    TextureManager::CreateCubeMap(sky_box, "SkyBox", true);

    // Create Texture for Post Processing
    TextureManager::CreateTexture2DMS(state.window->width, state.window->height, "MultiSampleAA");
...
Texture2DMS& TextureManager::CreateTexture2DMS(const int width, const int height, const std::string &texture_name) {
    Texture2DMS texture = Texture2DMS();
    texture.Generate(GL_RGB16F, GL_RGB, width, height, nullptr);
    texture.SetFilterParameters(GL_LINEAR, GL_LINEAR);
    texture.SetWrapParameters(GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
    texture2DMSs[texture_name] = texture;
    return texture2DMSs[texture_name];
}

Texture2DMS& TextureManager::GetTexture2DMS(const std::string &texture_name) {
    if (texture2DMSs.find(texture_name) != texture2DMSs.end()) {
        return texture2DMSs[texture_name];
    } else {
        Logger::Message(LogLevel::Error, "The texture <" + texture_name + "> which is not loaded in the application.");
        Logger::Message(LogLevel::Error, "Maybe you forget to use CreateTexture2DMS() in the TextureManager::Initialize().");
        exit(-1);
    }
}
