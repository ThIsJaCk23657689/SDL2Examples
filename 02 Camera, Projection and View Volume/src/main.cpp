...
int main(int argc, char** argv) {
    ...
    window = SDL_CreateWindow(
        "SDL2 Examples - 02 Camera, Projection and View Volume", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, window_width, window_height, flags);
    if (window == nullptr) {
...
void Render(const std::unique_ptr<Camera>& current_camera, float dt) {
    ...
    my_shader->SetVec3("objectColor", glm::vec3(0.2f));
    ...
    my_shader->SetVec3("objectColor", glm::vec3(42 / 255.0f, 219 / 255.0f, 89 / 255.0f));
    my_shader->SetMat4("model", model->Top());
    my_world.my_cube->Draw();
    model->Pop();
